...
<div class="container-fluid text-center">
    ...
    <div class="row">
        ...
        <div class="text-center col-sm-6">
            <div class="alert alert-warning" style="display:none;" id="noDataOfModePie">无数据，请更改查询条件或联系开发人员。</div>
            <div id="modePieChart" style="height:600px"></div>
        </div>
    </div>
    <div class="panel panel-default">
        <div class="panel-heading">
...
<script>
    ...
    if (list[1].length == 0) {
        $("#noDataOfModePie").css('display', 'block');
    } else {
        pieChart(list[1], "modePieChart", "登录方式分布", 2);
    }
    function pieChart(data, divId, chartName, legendIndex) {
        var legendData = [];
        var seriesData = [];
        for (var i = 0; i < data.length; i++) {
            var str = legendIndex == 1 ? data[i].logSource : data[i].logMode;
            legendData.push(str);
    ...
    if (list[2].length == 0) {
        ...
        trendChart(list[2], dateStrs, "dateTrendChart", "过去七天登录数趋势");
    }
    function trendChart(data, dateStrs, divId, chartName) {
        ...
        drawTrendChart(divId, chartName, legendData, xData, seriesData, "", "登录数");
    }
                    ...
                    success: function (data) {
                        ...
                        if (respCode == 0) {
                            ...
                            if (list[0].length == 0 || list[1].length == 0) {
                                ...
                                echarts.init(document.getElementById('modePieChart')).clear();
                                $("#sourcePieTable").bootstrapTable('load', []);
                                $("#modePieTable").bootstrapTable('load', []);
                            } else {
                                $("#noDataOfSourcePie").css('display', 'none');
                                $("#noDataOfModePie").css('display', 'none');
                                pieChart(list[0], "sourcePieChart", "渠道分布", 1);
                                pieChart(list[1], "modePieChart", "登录方式分布", 2);
                                $("#sourcePieTable").bootstrapTable('load', list[0]);
                                $("#modePieTable").bootstrapTable('load', list[1]);
                            }
                        }
    ...
    generateDataTable("sourcePieTable", [[{"field": "logSource"}, {"field": "logTimes"}, {"field": "logNum"}, {"field": "logTimesPerUser"}],
        [{"title": "登录渠道"}, {"title": "登录次数"}, {"title": "登录人数"}, {"title": "人均登录次数"}]])
    $("#sourcePieTable").bootstrapTable('load', list[0]);
    generateDataTable("modePieTable", [[{"field": "logMode"}, {"field": "logTimes"}, {"field": "logNum"}, {"field": "logTimesPerUser"}],
        [{"title": "登录方式"}, {"title": "登录次数"}, {"title": "登录人数"}, {"title": "人均登录次数"}]])
    $("#modePieTable").bootstrapTable('load', list[1]);
    generateDataTable("dateTrendTable", [[{"field": "logSource"}, {"field": "logNum7"}, {"field": "logNum6"}, {"field": "logNum5"},
        {"field": "logNum4"}, {"field": "logNum3"}, {"field": "logNum2"}, {"field": "logNum1"}],
        [{"title": "登录渠道"}, {"title": dateStrs[0]}, {"title": dateStrs[1]}, {"title": dateStrs[2]}, {"title": dateStrs[3]}, {"title": dateStrs[4]}, {"title": dateStrs[5]}, {"title": dateStrs[6]}]])
