...
<script type="text/javascript">
    var list =${pageClickDistributeList};
    var dateRange = '${startDate}' + " ~ " + '${endDate}';
    $("#dateRange").val(dateRange);
    if (list[0].length == 0) {
        $("#noDataOfCompanyPie").css('display', 'block');
    } else {
        pieChart(list[0], "companyPieChart", "扶贫相关页面活跃IP运营商分布");
    }
    if (list[1].length == 0) {
        $("#noDataOfLocationPie").css('display', 'block');
        $("#noDataOfLocationMap").css('display', 'block');
    } else {
        pieChart(list[1], "locationPieChart", "扶贫相关页面活跃IP归属地分布");
        mapChart(list[1], "locationMapChart", "扶贫相关页面活跃IP地理位置分布");
    }
    ...
    $(function () {
        $("#queryDate").click(function () {
            ...
            if ("" != startDate.trim() && "" != endDate.trim()) {
                ...
                $.ajax({
                    url: 'distributeAnalysisNumQuery',
